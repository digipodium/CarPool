...
public class CustomDialogRatingClass extends Dialog implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final TextView nameTxt = findViewById(R.id.txtName);
        final TextView rateTxt = findViewById(R.id.txtRate);
        final RatingBar ratingBar = findViewById(R.id.ratingBar);

        nameTxt.setText(String.format("Your rating for %s", u.getFname()));

        ratingBar.setOnRatingBarChangeListener((ratingBar1, rating, fromUser) -> {
            rateTxt.setText(String.format("%s", rating));
            rates = rating;
        });

        sendBtn = findViewById(R.id.ratingBtn);
